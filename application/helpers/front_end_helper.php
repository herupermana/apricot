...
function ambil_foto_galeri($id, $jumlah = false)
{
    ...
    $data = $CI->db->get_where('foto_galeri', ['id_galeri'=>$id]);
...
function ambil_foto_artikel($id, $jumlah = false)
{
    ...
    $CI->db->order_by('id_foto', 'DESC');
    $data = $CI->db->get_where('foto_artikel', ['id_artikel'=>$id]);
...
function faq($id = 0)
{
    ...
    if ($cache === false) {
        $query = $CI->db->where(['id'=>$id])->get('faq');
        if ($query->num_rows() > 0) {
...
function agenda($id = 0)
{
    ...
    if ($cache === false) {
        $query = $CI->db->where(['id'=>$id])->get('agenda');
        if ($query->num_rows() > 0) {
            $cache = $query->row_array();
        } else {
            $cache = [];
...
function group_banner($id)
{
    ...
    if ($cache === false) {
        $query = $CI->db->order_by('id', 'asc')->where(['id_group'=>$id])->get('banner_item');
        if ($query->num_rows() > 0) {
            $cache = $query->result_array();
        } else {
            $cache = [];
