...
class AN_Apricot extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->helper(['tambahan', 'custom_url', 'front_end', 'filter', 'text']);
        ...
        //aktifkan title
        $this->load->library(['title', 'artikel', 'galeri', 'biodata_web', 'menu', 'kategori', 'tag', 'newsticker', 'pihak_ketiga', 'pages']);

        //cari tema

        $cache_tema_aktif = $this->cache->file->get('tema_aktif');
        if ($cache_tema_aktif === false) {
            $tema_aktif = $this->db->get_where('tema', ['aktif'=>'Y']);
            $tema_aktif = ['jumlah'=>$tema_aktif->num_rows(), 'data'=>@$tema_aktif->row()];
