...
class AN_ajax_admin extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->helper(['filter', 'url', 'file', 'tambahan']);

        $this->load->library(['Slugify', 'zip']);

        $this->load->model('admin/otorisasi');
    }

    public function new_kategori($par = 0)
    {
        $nilai = ($this->input->post('value_k'));
        $slug = $this->slugify->slugify($nilai);

        if ($par == 0) {
            $query = $this->db->insert('kategori', ['nama_kategori'=>$nilai, 'slug_kategori'=>$slug]);
        } else {
            $query = $this->db->insert('kategori', ['nama_kategori'=>$nilai, 'slug_kategori'=>$slug, 'id_parent'=>$par]);
        }
        $last = $this->db->insert_id();
        $query2 = $this->db->get_where('kategori', ['id_kategori'=>$last]);
        $row = $query2->row();
    ...
    public function hapus_kategori()
    {
        ...
        $sql = $this->db->where(['id_kategori'=>$id])->update('kategori', ['terhapus'=>'Y']);

        //$query=$this->db->query("SELECT * FROM kategori WHERE id_kategori='$id' AND terhapus='N'");
        $query = $this->db->get_where('kategori', ['id_kategori'=>$id, 'terhapus'=>'N']);
    ...
    public function ubah_kategori()
    {
        ...
        $sql = $this->db->where(['id_kategori'=>$id])->update('kategori', ['nama_kategori'=>$nama, 'slug_kategori'=>$slug]);
    ...
    public function cek_new_username()
    {
        ...
        $query = $this->db->get_where('user', ['name_user'=>$username, 'terhapus'=>'N']);
        if ($query->num_rows() < 1) {
            echo 'ok';
        } else {
            echo 'terpakai';
    ...
    public function avatar_new()
    {
        ...
        if (!$this->upload->do_upload()) {
            ...
            $query = $this->db->insert('foto_user_tmp', ['nama_foto'=>$nama, 'token_foto'=>$token_foto, 'sesi_from'=>$sesi_form]);
            echo 'ok';
        }
    }

    public function delete_foto_temp()
    {
        $token_foto = ($this->input->post('foto_token'));
        //$query=$this->db->query("SELECT * FROM foto_user_tmp WHERE token_foto='$token_foto'");
        $query = $this->db->get_where('foto_user_tmp', ['token_foto'=>$token_foto]);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            //$query2=$this->db->query("DELETE FROM foto_user_tmp WHERE token_foto='$token_foto'");
            $query2 = $this->db->where(['token_foto'=>$token_foto])->delete('foto_user_tmp');
            if ($query) {
    ...
    public function new_admin()
    {
        ...
        $user = $this->db->get_where('user', ['name_user'=>$username, 'terhapus'=>'N']);
        ...
        if ($user->num_rows() > 0) {
            ...
            $savatar = $this->db->get_where('foto_user_tmp', ['sesi_from'=>$sessi]);
            if ($avatar == '0') {
                if ($savatar->num_rows() > 0) {
                    foreach ($savatar->result_array() as $row) {
                        ...
                        $this->db->where(['id_foto'=>$row['id_foto']])->delete('foto_user_tmp');
                        unlink(FCPATH.'an-component/media/upload-user-avatar/'.$row['nama_foto']);
                    }
                }
                $photo = 'default.jpg';
                ...
                $savatar = $this->db->delete('foto_user_tmp', ['sesi_from'=>$sessi]);
            }
            ...
            $query = $this->db->insert('user', ['name_user'=> $username,
                'nama_lengkap'                             => $nama,
                'email'                                    => $email,
    ...
    public function edit_username()
    {
        ...
        $cocok = $this->db->get_where('user', ['id_user'=>$id, 'name_user'=>$username, 'terhapus'=>'N']);

        if ($cocok->num_rows() > 0) {
            echo 'sama';
        } else {
            //$cari=$this->db->query("SELECT * FROM user WHERE name_user='$username' AND terhapus='N'");
            $cari = $this->db->get_where('user', ['name_user'=>$username, 'terhapus'=>'N']);

            if ($cari->num_rows() > 0) {
                echo 'terpakai';
            } else {
                //$query=$this->db->query("UPDATE user SET name_user='$username' WHERE id_user='$id'");
                $query = $this->db->where(['id_user'=>$id])->update('user', ['name_user'=>$username]);
                if ($query) {
                    echo 'ok';
                }
            }
    ...
    public function edit_fullname()
    {
        $id = ($this->input->post('id'));
        $name = trim(($this->input->post('name')));
        //$query=$this->db->query("UPDATE user SET nama_lengkap='$name' WHERE id_user='$id'");
        $query = $this->db->where(['id_user'=>$id])->update('user', ['nama_lengkap'=>$name]);
        if ($query) {
            echo 'ok';
        }
    }
    ...
    public function edit_email()
    {
        ...
        $query = $this->db->where(['id_user'=>$id])->update('user', ['email'=>$email]);
        if ($query) {
            echo 'ok';
        }
    }
    ...
    public function edit_level()
    {
        ...
        $query = $this->db->where(['id_user'=>$id])->update('user', ['level_user'=>$level]);
    ...
    public function edit_status()
    {
        ...
        $query = $this->db->where(['id_user'=>$id])->update('user', ['status_user'=>$status]);
        if ($query) {
            echo 'ok';
        }
    }
    ...
    public function edit_password()
    {
        ...
        $query = $this->db->where(['id_user'=>$id])->update('user', ['password_user'=>$password]);
        if ($query) {
            echo 'ok';
        }
    }
    ...
    public function avatar_update()
    {
        ...
        if (!$this->upload->do_upload()) {
            ...
            $query = $this->db->insert('foto_user_tmp', ['nama_foto'=>$nama, 'token_foto'=>$token_foto, 'id_user'=>$id]);
            echo 'ok';
        }
    }

    public function ganti_user_avatar()
    {
        $id = ($this->input->post('id'));
        //$cari=$this->db->query("SELECT * FROM foto_user_tmp WHERE id_user='$id' ORDER BY id_foto DESC");
        $cari = $this->db->order_by('id_foto', 'DESC')->get_where('foto_user_tmp', ['id_user'=>$id]);

        if ($cari->num_rows() > 0) {
            //$cari_foto_lama=$this->db->query("SELECT * FROM user WHERE id_user='$id'");
            $cari_foto_lama = $this->db->get_where('user', ['id_user'=>$id]);
            ...
            $query = $this->db->where(['id_user'=>$id])->update('user', ['avatar_user'=>$row->nama_foto]);

            //Hapus SEMUA yg punya ID user sama
            //$hapus=$this->db->query("DELETE FROM foto_user_tmp WHERE id_user='$id' ");
            $hapus = $this->db->delete('foto_user_tmp', ['id_user'=>$id]);
            if ($hapus) {
                echo 'ok';
            }
        }
    ...
    public function delete_user()
    {
        ...
        $query = $this->db->where(['id_user'=>$id])->update('user', ['terhapus'=>'Y']);
        if ($query) {
            echo 'ok';
        }
    }
    ...
    public function new_tag()
    {
        ...
        $query = $this->db->insert('tags', ['nama_tag'=>$tag, 'slug_tag'=>$slug, 'user_posting'=>$id]);
    ...
    public function edit_tag()
    {
        ...
        if ($modul == 'nama_tag') {
            $data = ['user_update'=>$id_user, 'nama_tag'=>$val, 'slug_tag'=>$aslug];
        } else {
            $data = ['user_update'=>$id_user, 'slug_tag'=>$val];
        }

        //$query=$this->db->query("UPDATE tags SET $exe='$val' $exe2 ,user_update='$id_user' WHERE id_tag='$id'");
        $query = $this->db->where(['id_tag'=>$id])->update('tags', $data);
    ...
    public function hapus_tag()
    {
        ...
        $query = $this->db->delete('tags', ['id_tag'=>$id]);
    ...
    public function foto_gallery_artikel()
    {
        $config = [
        ...
        if (!$this->upload->do_upload()) {
            ...
            if ($id == 0) {
                ...
                $query = $this->db->insert('foto_artikel_temp', ['nama_foto'=>$nama, 'token_foto'=>$token_foto, 'sesi_form'=>$sesi, 'id_user'=>$id]);
            } else {

                    //$query=$this->db->query("INSERT INTO foto_artikel (id_artikel,nama_foto,token_foto) VALUES ('$id','$nama','$token_foto')");
                $query = $this->db->insert('foto_artikel', ['id_artikel'=>$id, 'nama_foto'=>$nama, 'token_foto'=>$token_foto]);
            }

            $width_thumb = 300;
            //$cari_width=$this->db->query("SELECT width_thumb_artikel FROM informasi WHERE id='1'");
            $cari_width = $this->db->get_where('informasi', ['id'=>1]);
            ...
            if (!$this->image_lib->resize()) {
                ...
                if ($id == 0) {
                    ...
                    $search = $this->db->get_where('foto_artikel', ['token_foto'=>$token_foto]);
    ...
    public function set_featured_image()
    {
        ...
        $reset = $this->db->where(['featured'=>'Y', 'id_artikel'=>$artikel_id])->update('foto_artikel', ['featured'=>'N']);

        //$update=$this->db->query("UPDATE foto_artikel SET featured='Y' WHERE id_foto='$id'");
        $update = $this->db->where(['id_foto'=>$id])->update('foto_artikel', ['featured'=>'Y']);
        echo 'ok';
    }

    public function delete_foto_artikel_temp()
    {
        $token_foto = ($this->input->post('token'));

        //$query=$this->db->query("SELECT * FROM foto_artikel_temp WHERE token_foto='$token_foto' ");
        $query = $this->db->get_where('foto_artikel_temp', ['token_foto'=>$token_foto]);
        ...
        if ($query->num_rows() > 0) {
            ...
            $query2 = $this->db->delete('foto_artikel_temp', ['token_foto'=>$token_foto]);
    ...
    public function submit_artikel($i = 0)
    {
        ...
        if ($i == 0) {
            ...
            $search = $this->db->get_where('artikel', ['artikel_sesi_id'=>$sesi]);

            $data = ['artikel_judul'      => $judul,
                'artikel_isi'             => $isi,
            ...
            if ($search->num_rows() > 0) {
                $query = $this->db->where(['artikel_sesi_id'=>$sesi])->update('artikel', $data);
            } else {
                //belum ada di draft
            ...
            if ($sinc_artikel->num_rows() > 0) {
                ...
                $foto = $this->db->get_where('foto_artikel_temp', ['sesi_form'=>$sesi]);
                ...
                if ($foto->num_rows() > 0) {
                    foreach ($foto->result_array() as $data) {
                        ...
                        $this->db->delete('foto_artikel_temp', ['id_foto'=>$data['id_foto']]);
                    }
                }
            }
            ...
            $test = $this->db->get_where('artikel', ['artikel_id'=>$id]);
            $ddd = $test->row();
            ...
            $this->db->where(['artikel_id'=>$id])->update('artikel', $data);
    ...
    public function delete_artikel()
    {
        ...
        $artikel_sql = $this->db->delete('artikel', ['artikel_id'=>$id]);

        //$foto_artikel=$this->db->query("SELECT * FROM foto_artikel WHERE id_artikel='$id'");
        $foto_artikel = $this->db->get_where('foto_artikel', ['id_artikel'=>$id]);
        ...
        if ($foto_artikel->num_rows() > 0) {
            ...
            $this->db->delete('foto_artikel', ['id_artikel'=>$id]);
        }

        echo 'ok';
    }

    public function delete_atikel_foto()
    {
        @hapus_cache('artikel_headline');
        @hapus_cache('artikel_populer');
        $id = ($this->input->post('id'));
        ...
        $cari_foto = $this->db->get_where('foto_artikel', ['id_foto'=>$id]);
        ...
        if ($cari_foto->num_rows() > 0) {
            ...
            $query = $this->db->delete('foto_artikel', ['id_foto'=>$id]);
            if ($query) {
                echo 'sukses';
            }
        } else {
    ...
    public function delete_multi_photos()
    {
        @hapus_cache('artikel_headline');
        @hapus_cache('artikel_populer');
        ...
        foreach ($id as $value) {
            ...
            $cari = $this->db->get_where('foto_artikel', ['id_foto'=>$value]);
            if ($cari->num_rows() > 0) {
                $data = $cari->row();
                @hapus_cache('foto_artikel_'.$data->id_artikel);
                unlink(FCPATH.'an-component/media/upload-gambar-artikel/'.$data->nama_foto);
                ...
                $this->db->delete('foto_artikel', ['id_foto'=>$value]);
            }
        }
    ...
    public function delete_media_pendukung()
    {
        $nama = $this->input->post('nama');
    ...
    public function upload_multiple_pendukung()
    {
        $config = [
    ...
    public function simpanbiodata()
    {
        ...
        $query = $this->db->where(['id'=>1])->update('biodata', ['nama'=> $nama,
            'foto'                                                     => $fotobio,
    ...
    public function save_galeri()
    {
        ...
        if ($id == 0) {
            ...
            $foto = $this->db->order_by('id_foto', 'desc')->get_where('foto_galeri_temp', ['sesi_form'=>$sesi]);

            if ($foto->num_rows() > 0) {
                foreach ($foto->result_array() as $data) {
                    //$this->db->query("INSERT INTO foto_galeri (id_galeri,nama_foto) VALUES ('$new_id','$data[nama_foto]')");
                    $this->db->insert('foto_galeri', ['id_galeri'=>$new_id, 'nama_foto'=>$data['nama_foto']]);
                }
                //$this->db->query("DELETE FROM foto_galeri_temp WHERE sesi_form='$sesi'");
                $this->db->delete('foto_galeri_temp', ['sesi_form'=>$sesi]);
            }
            ...
            $query = $this->db->where(['galeri_id'=>$id])->update('galeri', $data);
    ...
    public function foto_galeri()
    {
        ...
        if (!$this->upload->do_upload()) {
            ...
            if ($id == 0) {
                ...
                $query = $this->db->insert('foto_galeri_temp', [
                    'nama_foto' => $nama,
                ...
                $query = $this->db->insert('foto_galeri', [
                    'id_galeri' => $id,
                    'nama_foto' => $nama,
                    'token_foto'=> $token_foto,
                ]);
            ...
            $cari_width = $this->db->select('width_thumb_galeri')->from('informasi')->where(['id'=>1])->get();
            if ($cari_width->num_rows() > 0) {
                $datW = $cari_width->row();
            ...
            if (!$this->image_lib->resize()) {
                ...
                if ($id == 0) {
                    ...
                    $search = $this->db->get_where('foto_galeri', ['token_foto'=>$token_foto]);
                    $dat = $search->row();
                    $dat_id = $dat->id_foto;
    ...
    public function delete_foto_galeri_temp()
    {
        $token_foto = ($this->input->post('token'));
        //$query=$this->db->query("SELECT * FROM foto_galeri_temp WHERE token_foto='$token_foto' ");
        $query = $this->db->get_where('foto_galeri_temp', ['token_foto'=>$token_foto]);
        if ($query->num_rows() > 0) {
            $data = $query->row();
            ...
            $query2 = $this->db->delete('foto_galeri_temp', ['token_foto'=>$token_foto]);
            unlink($file);
            unlink($thumbnail);
        }
    }

    public function delete_foto_galeri()
    {
        $id = ($this->input->post('id'));
        @hapus_cache('fotos_galeri_'.$id);
        //$search=$this->db->query("SELECT * FROM foto_galeri WHERE id_foto='$id'");
        $search = $this->db->get_where('foto_galeri', ['id_foto'=>$id]);
        if ($search->num_rows() > 0) {
            $data = $search->row();
            $file = FCPATH.'an-component/media/upload-galeri/'.$data->nama_foto;
            $thumbnail = FCPATH.'an-component/media/upload-galeri-thumbs/'.$data->nama_foto;
            //$query2=$this->db->query("DELETE FROM foto_galeri WHERE id_foto='$id' ");
            $query2 = $this->db->get_where('foto_galeri', ['id_foto'=>$id]);
            unlink($file);
            unlink($thumbnail);
        }
        echo 'ok';
    ...
    public function edit_deskripsi_galeri()
    {
        $id = ($this->input->post('id'));
        @hapus_cache('fotos_galeri_'.$id);
        $deskripsi = ($this->input->post('deskripsi'));
        //$query=$this->db->query("UPDATE foto_galeri SET deskripsi_foto='$deskripsi' WHERE id_foto='$id' ");
        $query = $this->db->where(['id_foto'=>$id])->update('foto_galeri', ['deskripsi_foto'=>$deskripsi]);
        echo 'ok';
    }
    ...
    public function delete_galeri()
    {
        ...
        $search = $this->db->get_where('galeri', ['galeri_id'=>$id]);
        if ($search->num_rows() > 0) {

            //$this->db->query("DELETE FROM galeri WHERE galeri_id='$id'");
            $this->db->delete('galeri', ['galeri_id'=>$id]);

            //$foto=$this->db->query("SELECT * FROM foto_galeri WHERE id_galeri='$id'");
            $foto = $this->db->get_where('foto_galeri', ['id_galeri'=>$id]);
            ...
            $query = $this->db->delete('foto_galeri', ['id_galeri'=>$id]);
        }
    ...
    public function delete_page()
    {
        if ($this->session->userdata('level_user') == 1) {
            ...
            $query = $this->db->delete('pages', ['page_id'=>$id]);
            echo 'ok';
        }
    }

    public function cek_code_menu()
    {
        $code = ($this->input->post('code'));
        //$query=$this->db->query("SELECT * FROM menu WHERE menu_code='$code'");
        $query = $this->db->get_where('menu', ['menu_code'=>$code]);
        if ($query->num_rows() > 0) {
            echo 'taken';
        } else {
            echo 'ok';
    ...
    public function insert_new_menu()
    {
        $id = ($this->input->post('id'));
        $nama = ($this->input->post('nama'));
        $url = ($this->input->post('url'));
        ...
        $query = $this->db->insert('menu_child', [
            'menu_id'          => $id,
        ...
        if ($query) {
            echo json_encode(['id'=>$this->db->insert_id(), 'nama'=>$nama, 'url'=>$url, 'target'=>$target, 'code'=>$code]);
        } else {
            echo $this->db->error();
        }
    }

    public function insert_new_sub_menu()
    {
        $id = ($this->input->post('id'));
        $nama = ($this->input->post('nama'));
        $url = ($this->input->post('url'));
        ...
        if ($query) {
            echo json_encode(['id'=>$this->db->insert_id(), 'nama'=>$nama, 'url'=>$url, 'target'=>$target, 'code'=>'']);
        } else {
            echo $this->db->error();
        }
    }

    public $collect_menu_child = [];

    public function delete_menu_child()
    {
        $menu_id = ($this->input->post('menu'));
        $id = ($this->input->post('id'));

        //$query=$this->db->query("SELECT * FROM menu_child WHERE menu_id='$menu_id' AND menu_child_id='$id'  ");
        $query = $this->db->get_where('menu_child', ['menu_id'=>$menu_id, 'menu_child_id'=>$id]);
        @hapus_cache('menu_one_level_vertikal');
        @hapus_cache('menu_one_level_horizontal');
    ...
    public function rec_delete_menu_child($menu, $parent)
    {
        ...
        $query2 = $this->db->select('menu_child_id')
                        ->from('menu_child')
                        ->where(['menu_id'=>$menu, 'menu_child_parent'=>$parent])
                        ->get();
    ...
    public function atur_menu()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $this->db->where(['menu_child_id'=>$key, 'menu_id'=>$id])
            ->update('menu_child', ['menu_child_parent'=>$value, 'aktif'=>'Y', 'posisi'=>$posisi]);
        }
    ...
    public function update_menu()
    {
        ...
        $query = $this->db->where(['menu_id'=>$parent, 'menu_child_id'=>$id])
                    ->update('menu_child', [
                        'menu_child_nama'  => $nama,
                        'menu_child_url'   => $url,
                        'menu_child_target'=> $target,
    ...
    public function hapus_group_menu()
    {
        $id = ($this->input->post('id'));
        //$data=$this->db->query("SELECT menu_code FROM menu WHERE menu_id='$id'");
        $data = $this->db->select('menu_code')->from('menu')->where(['menu_id'=>$id])->get();
        if ($data->num_rows() > 0) {
            $q = $data->row();
            if ($q->menu_code == 'vertical' or $q->menu_code == 'horizontal') {
                ...
                $this->db->delete('menu_child', ['menu_id'=>$id]);
                echo 'ok';
            }
        } else {
    ...
    public function new_kategori_galeri()
    {
        $nama = ($this->input->post('nama'));
        $slug = $this->slugify->slugify($nama);

        //$this->db->query("INSERT INTO kategori_galeri (nama_kategori,slug_kategori) VALUES ('$nama','$slug')");
        $this->db->insert('kategori_galeri', ['nama_kategori'=>$nama, 'slug_kategori'=>$slug]);

        @hapus_cache('one_level_category_galeri');
        echo json_encode(['id'=>$this->db->insert_id(), 'nama'=>$nama]);
    }

    public function edit_kategori_galeri($modul)
    {
        if ($this->session->userdata('level_user') == 1) {
            @hapus_cache('one_level_category_galeri');
            $id = ($this->input->post('id'));
            ...
            if ($modul == 'nama') {
                ...
                if ($nama == '[error]') {
                    ...
                    $query = $this->db->where(['id'=>$id])->update('kategori_galeri', [
                        'nama_kategori'=> $nama,
                        'slug_kategori'=> $slug,
                    ]);
                ...
                $this->db->where(['id'=>$id])->update('kategori_galeri', ['terhapus'=>'Y']);
                echo 'ok';
            } elseif ($modul == 'disable') {

                //$this->db->query("UPDATE kategori_galeri SET aktif='N'  WHERE id='$id'");
                $this->db->where(['id'=>$id])->update('kategori_galeri', ['aktif'=>'N']);
                echo 'ok';
            } elseif ($modul == 'enable') {
                //$this->db->query("UPDATE kategori_galeri SET aktif='Y'  WHERE id='$id'");
                $this->db->where(['id'=>$id])->update('kategori_galeri', ['aktif'=>'Y']);
                echo 'ok';
            }
        }
    }

    public function new_kategori_produk()
    {
        $nama = ($this->input->post('nama'));
        $parent = ($this->input->post('parent'));
        $slug = $this->slugify->slugify($nama);
        ...
        if ($parent > 0) {
            ...
            $query = $this->db->get_where('kategori_produk', ['id'=>$parent, 'aktif'=>'Y']);
            if ($query->num_rows() < 1) {
                $aktif = 'N';
            }
        }
        ...
        echo json_encode(['id'=>$this->db->insert_id(), 'nama'=>$nama, 'parent'=>$parent, 'aktif'=>$aktif]);
    }
    ...
    public function update_kategori_produk()
    {
        ...
        $this->db->where(['id'=>$id])->update('kategori_produk', ['nama_kategori'=>$nama, 'slug_kategori'=>$slug]);

        echo json_encode(['nama'=>$nama]);
    }

    protected $child_kategori_produk = [];

    public function aktif_kategori_produk()
    {
        $modul = ($this->input->post('modul'));
        $id = ($this->input->post('id'));
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($context == 'update') {
                ...
                $query2 = $this->db->get_where('kategori_produk', [
        ...
        if ($context == 'update') {
            ...
            foreach ($this->child_kategori_produk as $v) {
                ...
                $this->db->where(['id'=>$v])->update('kategori_produk', ['aktif'=>$aksi]);
            }
            ...
            foreach ($this->child_kategori_produk as $v) {
                ...
                $this->db->where(['id'=>$v])->update('kategori_produk', ['terhapus'=>'Y']);
            }
            echo json_encode($this->child_kategori_produk);
        }
    }
    ...
    protected function cari_child_kategori_p($id)
    {
        ...
        $query = $this->db->get_where('kategori_produk', ['parent_kategori'=>$id]);
        if ($query->num_rows() > 0) {
    ...
    public function new_banner_depan()
    {
        $data = [
        ...
        echo json_encode(array_merge($data, ['id'=>$this->db->insert_id()]));
    }

    public function update_banner_depan()
    {
        $id = ($this->input->post('id'));
        ...
        if ($modul == 'banner-header-field') {
            $data['header'] = $value;
        } elseif ($modul == 'banner-caption-field') {
            $data['caption'] = $value;
        } elseif ($modul == 'banner-link-field') {
            $data['link_href'] = $value;
        } elseif ($modul == 'banner-link-text-field') {
            $data['link_text'] = $value;
        } elseif ($modul == 'banner-gambar-field') {
            $data['gambar'] = $value;
        }
    ...
    public function delete_banner_depan()
    {
        $id = ($this->input->post('id'));
        $this->db->where('id', $id);
    ...
    public function update_news_ticker()
    {
        $modul = $this->input->post('modul');
        $id = ($this->input->post('id'));
        $value = ($this->input->post('nilai'));
        ...
        if ($modul == 'berita') {
            $this->db->where('id', $id);
            $this->db->update('news_ticker', ['berita'=>$value]);
        } elseif ($modul == 'link') {
            $this->db->where('id', $id);
            $this->db->update('news_ticker', ['link'=>$value]);
        }

        echo json_encode(['id'=>$id, 'value'=>$value]);
    }

    public function delete_news_ticker()
    {
        $id = $this->input->post('id');
    ...
    public function baca_inbox()
    {
        $id = $this->input->post('id');
        $query = $this->db->get_where('kontak_masuk', ['id'=>$id]);

        $data = $query->row_array();

        $this->db->where('id', $id);
        $this->db->update('kontak_masuk', ['dibaca'=>'Y']);
    ...
    public function distribusi_tema()
    {
        ...
        $tema = $this->db->get_where('tema', ['id'=>$id_tema]);
        ...
        echo json_encode(['tema'=>$id_tema, 'versi'=>$versi, 'gambar'=>$gmb, 'link'=>base_url('distribusi_tema/'.$tema->nama_tema.'.zip')]);
    }

    public function tema_baru()
    {
        $nama = trim($this->input->post('nama'));
        if (strlen($nama) < 7) {
            echo json_encode(['status'=>'error', 'pesan'=>'Karakter kurang! Minimal 7 huruf']);
        } else {
            $cari = $this->db->get_where('tema', ['nama_tema'=>$nama]);
            if ($cari->num_rows() > 0) {
                echo json_encode(['status'=>'error', 'pesan'=>'Nama ini sudah digunakan']);
            } else {
                $viewsPHP = [
                    'about_us',
                ...
                $this->db->insert('tema', ['nama_tema'=>$nama, 'versi'=>'1.0', 'aktif'=>'N', 'screenshot'=>'screenshot.jpg']);

                echo json_encode(['status'=>'success', 'pesan'=>"<strong>Workspace Tema anda telah berhasil dibuat!</strong><br> Silahkan masukan file CSS, Js, dan asset asset lainnya didalam folder <em>'an-theme/$nama/assets/'</em><br> Dan silahkan edit tema anda didalam folder <em>'application/views/$nama/'</em>"]);
            }
        }
    }

    public function aktifkan_tema()
    {
        $id = $this->input->post('id');

        $tema = $this->db->get_where('tema', ['id'=>$id]);
        if ($tema->num_rows() > 0) {
            $data = $tema->row();
            $nama = $data->nama_tema;
            ...
            if (file_exists($assets) && file_exists($views)) {
                ...
                echo json_encode(['status'=>'error', 'pesan'=>'Tema Korup. Direktori tema tidak ditemukan']);
            }
        } else {
            echo json_encode(['status'=>'error', 'pesan'=>'Tema tidak ada didatabase']);
        }
    }
    ...
    public function hapus_tema()
    {
        ...
        if ($tema->num_rows() > 0) {
            ...
            $this->db->delete('tema');
            @hapus_cache('tema_aktif');
            echo json_encode(['status'=>'success', 'pesan'=>'Berhasil Menghapus tema']);
        } else {
            echo json_encode(['status'=>'error', 'pesan'=>'Tema tidak ada didatabase']);
        }
    }

    public function upload_tema_baru()
    {
            ...
            if ($buka === true) {
                ...
                if (file_exists($folder_file)) {
                    ...
                    if (file_exists($json_file)) {
                        ...
                        $cari = $this->db->get_where('tema', ['nama_tema'=>$info_tema['nama']]);
                        if ($cari->num_rows() > 0) {
                            //hapus folder
                            delete_files($folder_file, true);
                            rmdir($folder_file);
                            echo json_encode(['status'=>'error', 'pesan'=>'Tema dengan nama yang sama telah ditemukan masih terinstal. Uninstal dulu tema sebelumnya']);
                            exit;
                        }
                ...
                //buat path view dan aseets
                        ...
                        $this->db->insert('tema', ['nama_tema'=>$info_tema['nama'], 'versi'=>$info_tema['versi'], 'aktif'=>'N', 'author'=>$info_tema['author'], 'web'=>$info_tema['web'], 'screenshot'=>$info_tema['screenshot']]);

                        echo json_encode(['status'=>'success', 'pesan'=>'Tema Terinstal']);
                    } else {
                        delete_files(FCPATH.'upload-tema', true);
                        echo json_encode(['status'=>'error', 'pesan'=>'File json tidak ditemukan']);
                    }
                } else {
                    delete_files(FCPATH.'upload-tema', true);
                    echo json_encode(['status'=>'error', 'pesan'=>'Gagal terekstract']);
                }

                //echo json_encode(array("status"=>"success","pesan"=>"$nama_file"));
            } else {
                echo json_encode(['status'=>'error', 'pesan'=>'File tidak bisa dibuka']);
            }
    ...
    public function update_smtp()
    {
        ...
        $input = ['user'=>$user, 'host'=>$host, 'password'=>$password, 'nama'=>$nama, 'port'=>$port, 'ssl_connection'=>$ssl_connection];

        if ($password == '' || $password == ' ') {
            $input = ['user'=>$user, 'host'=>$host, 'nama'=>$nama, 'port'=>$port, 'ssl_connection'=>$ssl_connection];
        }
    ...
    public function file_download()
    {
        ...
        if (!$this->upload->do_upload('userfile')) {
            ...
            $nama = $this->upload->data('file_name');
            $this->db->insert('download_temp', ['file'=>$nama, 'token_file'=>$token, 'sesi_form'=>$sesi]);
            echo 'berhasil';
        }
    }

    public function delete_file_download_temp()
    {
    ...
    public function proses_input_download()
    {
        $id = $this->input->post('id');
        $nama = $this->input->post('nama');
        $slug = $this->slugify->slugify($nama);
        ...
        if ($id == '0' || $download->num_rows() < 1) {
            //baru
            $filee = $filee->row();
            $this->db->insert('download', ['nama_file'=> $nama,
                'file'                                => $filee->file,
                'deskripsi'                           => $deskripsi,
                'slug'                                => $slug,
            ]);
            ...
            if ($filee->num_rows() > 0) {
                ...
                $this->db->where('id', $id)->update('download', ['nama_file'=> $nama,
                    'file'                                                  => $filee->file,
                    'deskripsi'                                             => $deskripsi,
                    'slug'                                                  => $slug,
                ]);
    ...
    public function hapus_download()
    {
        $id = $this->input->post('id');
    ...
    public function proses_update_password()
    {
        ...
        $user = $this->db->get_where('user', ['id_user'=>$id]);

        if ($user->num_rows() > 0) {
            $user = $user->row_array();
            if (password_verify($password_lama, $user['password_user'])) {
                $this->db->where(['id_user'=>$id])->update('user', ['password_user'=>$password_baru]);
                $this->session->set_userdata('password_user', $password_baru);
                echo json_encode(['status'=>'success']);
            } else {
                echo json_encode(['status'=>'error', 'pesan'=>'Password lama Anda salah']);
            }
        } else {
    ...
    public function input_agenda()
    {
        ...
        if ($id == '0') {
            $this->db->insert('agenda', ['judul'=>$judul, 'slug'=>$slug, 'deskripsi'=>$deskripsi, 'tanggal_mulai'=>$tanggal_mulai, 'tanggal_selesai'=>$tanggal_selesai, 'foto'=>$foto]);
            $id = $this->db->insert_id();
            echo json_encode(['status'=>'baru', 'id'=>$id]);
            @hapus_cache('agenda_'.$id);
        } else {
            $this->db->where('id', $id)->update('agenda', ['judul'=>$judul, 'slug'=>$slug, 'deskripsi'=>$deskripsi, 'tanggal_mulai'=>$tanggal_mulai, 'tanggal_selesai'=>$tanggal_selesai, 'foto'=>$foto]);
            @hapus_cache('agenda_'.$id);
    ...
    public function hapus_tags_cache()
    {
        $daftar_file = get_dir_file_info(FCPATH.'application/cache', false);
        foreach ($daftar_file as $file) {
    ...
    public function hapus_group_banner()
    {
        $id = $this->input->post('id');
        @hapus_cache('group_banner_'.$id);
        $this->db->trans_begin();
        $this->db->delete('group_banner', ['id_group'=>$id]);
        $this->db->delete('banner_item', ['id_group'=>$id]);
    ...
    public function submit_item_banner()
    {
        $id = $this->input->post('id');
        $nama = $this->input->post('nama');
        $data = $this->input->post('data');
        ...
        if (!($id > 0)) {
            //new
            $this->db->insert('group_banner', [
                'nama'   => $nama,
                'tanggal'=> $tanggal,
            ]);
            $id = $this->db->insert_id();
        } else {
            $this->db->where(['id_group'=>$id])->update('group_banner', [
                'nama'=> $nama,
            ]);
        }
