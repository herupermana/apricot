...
class AN_admin extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        //panggil helper
        $this->load->helper(['filter', 'url', 'text']);
    ...
    public function reply($id)
    {
        if (!$this->login) {
            ...
            $pesan = $this->db->get_where('kontak_masuk', ['id'=>$id]);
            if ($pesan->num_rows() > 0) {
                $data = [
                    'avatar'     => $this->avatar_user,
                    'path_avatar'=> base_url().'an-component/media/upload-user-avatar/'.$this->avatar_user,
    ...
    public function faq($id = 0)
    {
        if (!$this->login) {
            ...
            $faq = $this->db->get_where('faq', ['id'=>$id]);
    ...
    public function banner($id = 0)
    {
        if (!$this->login) {
            ...
            $cari = $this->db->get_where('group_banner', ['id_group'=>$id]);
            if ($id > 0 && !($cari->num_rows() > 0)) {
                show_404();
                exit();
            } else {
                $banner = $cari->row_array();
                $item_banner = $this->db->order_by('id', 'asc')->get_where('banner_item', ['id_group'=>$id])->result_array();
            }
    ...
    public function pengaturan_tema()
    {
        if (!$this->login) {
            redirect('admin/login');
        } else {
    ...
    public function smtp_email()
    {
        if (!$this->login) {
            ...
            $smtp = $this->db->get_where('smtp_email', ['id'=>'1']);
            if ($smtp->num_rows() < 1) {
                $this->db->insert('smtp_email', ['id'=>'1']);
                $smtp = $this->db->get_where('smtp_email', ['id'=>'1']);
            }
    ...
    public function proseslogin()
    {
        if ($this->input->post()) {
            ...
            $cari = $this->db->get_where('user', ['name_user'=>$user, 'status_user'=>'Y', 'terhapus'=>'N']);
    ...
    {
        $data = ['login', 'id_user', 'name_user', 'password_user', 'level_user', 'random_filemanager_key'];
